...
public partial class MVPanel : Gtk.Bin
	{
		...
		{ 
			...
			frame.Put(ctrl, 0, 0);
			frame.Put(new Button(Stock.Ok),0,0);
			frame.ShowAll();
			rev.Add(frame);
			...
			if (this.currCtrl!=null)
			{
				if (currCtrl is EventBox)
				{
					...
					if (fr is Fixed)
					{
						...
						re = GetMovingBox((currCtrl as EventBox).Name+"Clone", (mv as  MVObject).Caption, (mv as MVObject).ELType,(mv as MVObject).width,(mv as MVObject).height);
					}
				}
			}
			if (re==null)
