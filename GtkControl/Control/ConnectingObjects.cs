	...
	public class SequenceFlow : ConnectingObject
    {
        ...
        public override void Paint (Context gr)
		{
			...
			case ConditionType.Expression:
			{
				...
				var x1 = WayPoints [0].X + arrow_lenght * 2 * Math.Cos (angle);
				var y1 = WayPoints [0].Y + arrow_lenght * 2 * Math.Sin (angle);
				gr.MoveTo (WayPoints [0]);
				gr.LineTo(p1);
				gr.LineTo(x1, y1);
				gr.LineTo(p2);
			...
			case ConditionType.Default:
				gr.NewPath ();
				double angleD = Math.Atan2 (WayPoints [0].Y - WayPoints [1].Y, WayPoints [0].X - WayPoints [1].X) + Math.PI;
				var arrow_lenghtD = 10;
				var arrow_degreesD = Math.PI/4;
				var x3 = WayPoints [0].X + arrow_lenghtD * 2 * Math.Cos (angleD);
				var y3 = WayPoints [0].Y + arrow_lenghtD * 2 * Math.Sin (angleD);
				p1.X = x3 + arrow_lenghtD * Math.Cos (angleD + arrow_degreesD);
				p1.Y = y3 + arrow_lenghtD * Math.Sin (angleD + arrow_degreesD);
				p2.X = x3 - arrow_lenghtD * Math.Cos (angleD + arrow_degreesD);//+ arrow_lenghtD * Math.Cos (angleD + arrow_degreesD);
				p2.Y = y3 - arrow_lenghtD * Math.Sin (angleD + arrow_degreesD);//+ arrow_lenghtD * Math.Sin (angleD + arrow_degreesD);
